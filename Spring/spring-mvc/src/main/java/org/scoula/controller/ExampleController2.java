...
public class ExampleController2 {
    ...
    * @ModelAttribute*/
    ...
    //컨트롤러 메서드가 void형태면
    //요청 URL을 기준으로 뷰 이름을 추론하여 해당 뷰를 찾음
    @GetMapping("/index")
    public void get() {
        log.info("get ================> 호출됨");
    }
    ...
    * - 기본적으로 해당 파라미터가 없으면 예외 발생
    * ->
    * */

    //http://localhost:8080/example02?id=user&password=1234
    @GetMapping("/example02")
    public String example01(@RequestParam(name = "id", defaultValue = "defaultUser") String id, @RequestParam(name = "password", required = false) String password, Model model) {
...
//    @RequestParam(name = "names") ArrayList<String> names,
    public String example01(
        ...
        model.addAttribute("messasge", Arrays.asList(names));
    ...
    * */
    ...
    //http://localhost:8080/example04/user (String인 경우)
